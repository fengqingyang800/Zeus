...
// 对requestAnimationFrame方法做兼容处理  不支持requestAnimationFrame方法则用setTimeout模拟
...
// 公用方法
    var utils = (function () {
        ...
        // 取得浏览器厂商的值  如果有则返回浏览器厂商  不支持则直接返回false
        ...
        // 前缀预处理 不支持则直接返回false  支持返回对应的带前缀的属性
         ...
         * start - 初始位置
         * time - 持续时间
         * lowerMargin - maxScrollY
         * wrapperSize - wrapperOffsetHeight
         * deceleration - 阻尼系数
         * */
        me.momentum = function (current, start, time, lowerMargin, wrapperSize, deceleration) {
            var distance = current - start,// 运动的距离  当前位置-开始位置
                speed = Math.abs(distance) / time, // 速度  运动的距离／运动时间
                destination,
                duration;
            // 默认减速系数是 0.0006
        ...
        me.extend(me, {
            hasTransform: _transform !== false,// 只要支持 transform 则 hasTransform为true
            hasPerspective: _prefixStyle('perspective') in _elementStyle, // 支持3D视图
            hasTouch: 'ontouchstart' in window, // 是否支持touch事件
            hasPointer: !!(window.PointerEvent || window.MSPointerEvent), // IE10 is prefixed 是否支持point事件
            hasTransition: _prefixStyle('transition') in _elementStyle // 是否支持transition
        });
    ...
    function IScroll (el, options) {
        ...
        // 滑块样式 - 缓存起来提高效率
        this.scrollerStyle = this.scroller.style;		// 缓存样式，提高效率
        ...
        // Some defaults  滑块的左上角坐标
        this.x = 0;
        this.y = 0;
        this.directionX = 0;
        this.directionY = 0;
        this._events = {};
        // 将IScroll初始化完成后即刻刷新，修正IScroll的参数值
        this._init();
        this.refresh();
        // 将页面滚动到坐标（startX,startY）后，启用IScroll
    ...
    IScroll.prototype = {
        ...
        refresh: function () {
            // 容器的高度 - 如果有border则加上border的高度
            var rf = this.wrapper.offsetHeight;		// 强制重绘
            // 可见区域的宽度和高度 - 如果有滚动条则减去滚动条的高度
            ...
            // 如果没有垂直滚动  则将maxScrollY设置为0  滑块的高度设置为容器的高度
        ...
        scrollTo: function (x, y, time, easing) {
            ...
            var transitionType = this.options.useTransition && easing.style; // 'cubic-bezier(0.1, 0.57, 0.1, 1)'

            // 1、有时间并且有transitionType 2、没有时间
        ...
        _initEvents: function (remove) {
            ...
            // 绑定方向改变事件和 resize事件  orientationchange - 设备水平或者垂直旋转时触发
            eventType(window, 'orientationchange', this);
            eventType(window, 'resize', this);

            // 如果有click则绑定click事件，不建议用click事件，移动端有300ms延迟  注意这个click竟然是捕获阶段触发
            if ( this.options.click ) {
